...
// i2c slave memory and index
static uint8_t index_slave;
static uint8_t slave_memory[256];
static bool slave_first_byte = false;
...
static bool i2c__handle_state_machine(i2c_s *i2c) {
  enum {
    ...
    // Slave Sates:
    I2C__STATE_SLAVE_START = 0x60,
    I2C__STATE_SLAVE_WRITE = 0x80,
    I2C__STATE_SLAVE_REPEAT_START = 0xA0,
    I2C__STATE_SLAVE_READ = 0xA8,
  };
  ...
  switch (i2c_state) {
  // slave machine states
  case I2C__STATE_SLAVE_START:
    index_slave = 0;
    slave_first_byte = false;
    i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;

  case I2C__STATE_SLAVE_WRITE:
    if (!slave_first_byte) {
      index_slave = lpc_i2c->DAT;
      slave_first_byte = true;
    } else {
      const uint8_t data = lpc_i2c->DAT;
      slave_memory[index_slave] = data;
    }
  ...
  case I2C__STATE_SLAVE_REPEAT_START:
    slave_first_byte = false;
    i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;

  case I2C__STATE_SLAVE_READ:
    lpc_i2c->DAT = slave_memory[index_slave];
    i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;
